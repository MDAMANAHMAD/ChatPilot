/**
 * AI INTELLIGENCE HUB (AIPanel)
 * 
 * Provides real-time AI capabilities to the user. Features include:
 * - Smart Reply Suggestions: Clicking a suggestion populates the chat input.
 * - Auto-Pilot Mode: Toggle for autonomous response sending ({'>'}90% confidence).
 * - Live Status Indicators: Displays AI model health and latency.
 */
    ...
    <div className="w-[30%] min-w-[320px] max-w-[400px] bg-pilot-surface/80 border-l border-pilot-border h-full flex flex-col relative z-30 backdrop-blur-xl animate-in slide-in-from-right duration-500 shadow-[-10px_0_30px_rgba(0,0,0,0.3)]">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10">
        ...
        <div className="p-6 space-y-8">
            ...
            {/* AUTO-PILOT CONTROL
                When active, the backend determines if AI should send a message 
                without user intervention based on its internal confidence score.
            */}
            <div className="pilot-glass p-5 rounded-2xl relative overflow-hidden group">
                ...
                {autoMode && (
                    <div className="mt-4 pt-4 border-t border-white/5 flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-pilot-accent">Min Confidence: 90%</span>
                    </div>
                )}
            </div>

            {/* SUGGESTIONS ENGINE
                Provides up to 3 Smart Reply suggestions generated by the Gemini API.
            */}
            <div>
                ...
                {loading ? (
                    ...
                    <div className="space-y-3">
                        {suggestions.map((suggestion, idx) => (
                            <button
                                ...
                                className="w-full text-left p-4 rounded-xl bg-white/5 hover:bg-pilot-primary/20 text-[14px] text-white border border-white/5 hover:border-pilot-primary/30 transition-all duration-300 group relative"
                            >
                    ...
                    <div className="text-center py-16 px-6 bg-pilot-bg/20 rounded-2xl border border-dashed border-pilot-border/50">
                        <MessageSquare size={40} className="text-pilot-secondary mx-auto mb-4 opacity-20" />
                        <p className="text-[13px] text-pilot-secondary font-medium">Awaiting incoming logs...</p>
                    </div>
                )}
            </div>
        </div>
      ...
      <div className="p-6 bg-pilot-header/30 border-t border-pilot-border">
           <div className="flex items-center justify-between">
                ...
                <div className="text-[10px] text-pilot-secondary font-mono">LATENCY: ~120ms</div>
           </div>
      </div>
    </div>
  );
